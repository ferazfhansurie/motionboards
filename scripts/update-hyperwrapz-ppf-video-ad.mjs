...
const AD_ID = "52549829975148";

const SC = "/private/tmp/claude-501/-Users-faeez-motionboards/9a45d38a-94db-4d02-9904-4e90fbc57221/scratchpad";
const VIDEO = path.join(SC, "ppf_video_ad_corrected.mp4");
const THUMB = path.join(SC, "ppf_video_thumb_corrected.jpg");

// AIDOCAC structure
const MESSAGE = [
  "Kereta baru, tapi risau kena calar dalam bulan pertama?",                                            // Interest — audience-specific pain
  "Cat yang calar boleh jejaskan nilai kereta anda - dan selalunya kita tak perasan sampai dah terlambat. Dengan Full Car PPF 8.5mil, cat kereta anda kekal licin dan terlindung daripada calar halus, kesan batu jalan dan cuaca.", // Desire — agitate + benefit + transformation
  "Full Car PPF 8.5mil dari RM3,300. Promo Merdeka terhad bulan ini.",                                   // Offer — clear value + urgency
  "Hyperwrapz & Detailing - 19 tahun dalam industri automotive protection di Klang.",                    // Credential
  "WhatsApp Hyperwrapz & Detailing sekarang untuk book slot anda.",                                      // Action — single clear CTA
].join("\n\n");
...
async function main() {
  ...
  const creative = await graph(`${ACCOUNT}/adcreatives`, "POST", {
    name: "PPF Real Presenter Video (Corrected, AIDOCAC) | Facebook | WhatsApp CTA",
  ...
  console.log("new creative:", creative.id);

  const updated = await graph(`${AD_ID}`, "POST", { creative: { creative_id: creative.id } });
  console.log("ad updated:", JSON.stringify(updated));

  console.log(JSON.stringify({ ad: AD_ID, newCreative: creative.id, newVideoId: videoId }, null, 2));
}
