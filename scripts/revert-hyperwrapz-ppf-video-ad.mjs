...
// same AIDOCAC message as before
const MESSAGE = [
...
async function main() {
  ...
  const creative = await graph(`${ACCOUNT}/adcreatives`, "POST", {
    name: "PPF Real Presenter Video (Original) | Facebook | WhatsApp CTA",
  ...
  // check if the current ad is archived before attempting update
  const current = await graph(`${AD_ID}?fields=effective_status,adset_id`, "GET");
  console.log("current ad status:", JSON.stringify(current));

  if (current.effective_status === "ARCHIVED") {
    const newAd = await graph(`${ACCOUNT}/ads`, "POST", {
      name: "PPF Real Presenter Video (Original) | WhatsApp CTA",
      adset_id: current.adset_id,
      creative: { creative_id: creative.id },
      status: "ACTIVE",
    });
    console.log("ad was archived, created new ad:", JSON.stringify(newAd));
  } else {
